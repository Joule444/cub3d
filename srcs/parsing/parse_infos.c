...
/*   Updated: 2023/09/15 11:46:43 by jthuysba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	set_info(char **buff, t_parsing *data)
{
	...
	else if (ft_strcmp(buff[0], "C") == 0)
	{
		if (data->c != -1)
		{
			printf("\033[31;01mInfos error :\033[00m Multiple C textures paths !\n");
			return (1);
		}
	...
	else if (ft_strcmp(buff[0], "F") == 0)
	{
		if (data->f != -1)
		{
			printf("\033[31;01mInfos error :\033[00m Multiple F textures paths !\n");
			return (1);
		}
	...
	while (line && !check_full_elem(&cub->parsing))
	{
		...
		{	
			if (parse_elem(line, &cub->parsing) != 0)
				return (skip_file(line, cub->parsing.fd),
